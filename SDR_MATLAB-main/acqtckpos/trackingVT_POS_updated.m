...
disp('Running trackingVT_POS_updated...');
...
disp(['Acquired.sv(svindex) = ', num2str(Acquired.sv(svindex))]);
disp(['msStartTckVT = ', num2str(msStartTckVT)]);
disp(['Size of TckResultCT(Acquired.sv(svindex)).codeFreq = ', num2str(size(TckResultCT(Acquired.sv(svindex)).codeFreq, 2))]);
maxIndex = size(TckResultCT(Acquired.sv(svindex)).codeFreq, 2);
msStartTckVT = min(msStartTckVT, maxIndex); % 确保索引不超出范围
disp(['Adjusted msStartTckVT = ', num2str(msStartTckVT)]);
...
toc
%% ======== Task 2: 绘制相关数据 ========
disp('Debug: Starting Task 2 Plotting...');
...
for i = 1:length(sv)
    ...
    % 计算相关功率 (I^2 + Q^2)
    correlationPower = TckResultVT(prn).P_i.^2 + TckResultVT(prn).P_q.^2;
    
    % 计算平均相关功率，用于绘图
    avgCorrelation = mean(correlationPower, 1);
    
    % 绘制相关函数
    plot(1:length(avgCorrelation), avgCorrelation, 'LineWidth', 2);
    
    % 找到最大相关峰值并标注
    [maxVal, maxIdx] = max(avgCorrelation);
    text(maxIdx, maxVal, ['PRN ', num2str(prn)], 'FontSize', 12, 'Color', 'r');
end

xlabel('Code Phase');
ylabel('Correlation Power');
title('Task 2: Correlation Function for Tracked Satellites');
legend(arrayfun(@(x) ['PRN ', num2str(x)], sv, 'UniformOutput', false));
grid on;
...
for i = 1:length(sv)
    ...
    % 获取 I/Q 数据
    I = TckResultVT(prn).P_i;
    Q = TckResultVT(prn).P_q;
    
    % 绘制星座图
    plot(I, Q, '.', 'DisplayName', ['PRN ', num2str(prn)]);
end

xlabel('In-phase (I)');
ylabel('Quadrature (Q)');
title('Task 2: IQ Constellation Diagram');
legend;
grid on;

fprintf('Task 2: Correlation and IQ plots generated.\n');
